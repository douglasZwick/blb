...
namespace UnityEngine.UI
{
  ...
  {
    [SerializeField]
    private Color shadowColor = new(0f, 0f, 0f, 0.5f);

    [SerializeField]
    private Vector2 shadowDistance = new(1f, -1f);
    ...
    void DropShadowEffect(List<UIVertex> verts)
    {
      ...
      List<UIVertex> vertsCopy = new(verts);
      verts.Clear();
      ...
      for (int i = 0; i < iterations; i++)
      {
        for (int v = 0; v < count; v++)
        {
          ...
          color.a = (byte)((float)color.a / iterations);
          vt.color = color;
          verts.Add(vt);
        }
      }
